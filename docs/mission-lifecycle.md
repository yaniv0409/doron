...
8. enqueues post-mission skill maintenance when enabled
...
Browser navigation uses a dedicated 15-second timeout by default. The mission-scoped browser page is reused by `browser_text`, while `browser_open` creates short-lived Playwright sessions per URL and closes them after extraction. Browser timeouts are returned as recoverable tool errors, and browser-stage progress is written during mission-scoped navigation so a stuck page can be distinguished from a stalled model call. `browser_open` also emits batch-level start/completion and per-URL success/failure events.
